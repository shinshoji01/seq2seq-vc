...
class M2MVTN(torch.nn.Module): # many-to-many VTN
    def __init__(
        ...
        idim_encoder = idim+acdim if conditiontype=="concat" else idim
        # define encoder
        ...
        idim_decoder = odim+acdim if conditiontype=="concat" else odim
        ...
        # Condition for Many-to-Many Conversion
    ...
    def forward(self, xs, ilens, ys, labels, olens, xembs, yembs, spembs=None, *args, **kwargs):
        ...
        if self.conditiontype=="add":
            ...
            ys_in = torch.cat([ys_in, yembs], axis=-1)
        elif self.conditiontype=="nocondition":
            pass
        zs, _ = self.decoder(ys_in, y_masks, hs_int, hs_masks)
        # zs = zs[:,:,:-1*yembs.shape[-1]] # Decouple
        
        # (B, Lmax//r, odim * r) -> (B, Lmax//r * r, odim)
        before_outs = self.feat_out(zs).view(zs.size(0), -1, self.odim)
        # before_outs = before_outs[:,:,:100] # Decouple
    ...
    def inference(self, x, inference_args, xembs, yembs, spemb=None, *args, **kwargs):
        ...
        def ycondition(ys):
            ...
            if self.conditiontype=="add":
                ...
                a = torch.cat([ys, condition], axis=-1)
            elif self.conditiontype=="nocondition":
                a = ys
            return a
        ...
        while True:
            ...
            z, z_cache = self.decoder.forward_one_step(
                ycondition(ys), y_masks, hs, cache=z_cache
            )  # (B, adim)
            outs += [
