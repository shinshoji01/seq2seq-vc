...
class ARM2MVCADVTrainer(Trainer):
    ...
    def _train_step(self, batch):
        ...
        accentlabels = batch["accents"].to(self.device)
        ...
        (
            ...
            (advfeatures, accenttargets),
        ) = self.model(xs, ilens, ys, labels, olens, xembs, yembs, accentlabels)

        # seq2seq loss
        losses = self.criterion["Seq2SeqAdvLoss"](
            after_outs, before_outs, logits, ys_, labels_, olens_, advfeatures, accenttargets
        )
        l1_loss, bce_loss, adv_loss = losses
        gen_loss = l1_loss + bce_loss + adv_loss
    ...
    def _genearete_and_save_intermediate_result(self, batch):
        ...
        for idx, (x, y, olen, spemb, xemb, yemb) in enumerate(zip(xs, ys, olens, spembs, xembs, yembs)):
            start_time = time.time()
            config_inference = self.config["inference"].copy()
            config_inference["maxlenratio"] = config_inference["maxlenratio"]*y.shape[0]/x.shape[0]
            outs, probs, att_ws, _ = self.model.inference(
                x, self.config["inference"], xemb.reshape(1,-1), yemb.reshape(1,-1), spemb=spemb,
            )
            logging.info(
